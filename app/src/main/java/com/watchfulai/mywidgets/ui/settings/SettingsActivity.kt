package com.watchfulai.mywidgets.ui.settings
...
import com.watchfulai.mywidgets.data.AppTheme
...
class SettingsActivity : LocaleAwareActivity() {
    ...
    private fun requestWidgetLocaleRefresh() {
        com.watchfulai.mywidgets.data.ActiveWidgetsTracker.KNOWN_PROVIDERS.forEach { provider ->
            val ids = AppWidgetManager.getInstance(applicationContext)
                .getAppWidgetIds(ComponentName(applicationContext, provider))
            if (ids.isNotEmpty()) applicationContext.sendBroadcast(
                Intent(applicationContext, provider).apply {
